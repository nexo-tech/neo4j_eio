...
let example_variable_length_paths session =
  ...
  | Ok () ->
      ...
      (match Query_builder.execute
         (Query_builder.raw (Printf.sprintf "MATCH (start:%s {name: 'Alice'})-[:KNOWS*1..3]->(connected:%s) RETURN DISTINCT connected.name AS name ORDER BY name" label label))
         session with
       | Ok records ->
...
let example_pattern_comprehension session =
  ...
  | Ok () ->
      ...
      let _ = Query_builder.execute_unit
        ...
        session in
      let _ = Query_builder.execute_unit
        (Query_builder.raw "MATCH (n:Project) DETACH DELETE n")
        session in
      Printf.printf "  ✓ Cleanup completed\n"
...
let example_collect_aggregation session =
  ...
  | Ok () ->
      ...
      let _ = Query_builder.execute_unit
        ...
        session in
      let _ = Query_builder.execute_unit
        (Query_builder.raw "MATCH (n:Book) DETACH DELETE n")
        session in
      Printf.printf "  ✓ Cleanup completed\n"
